...
for k=1:n_layers
    ...
    for i=1:(n_mats)
        if mat_index == cell2mat(material_input_data(i+1,1))%check that this is the indexed material
            layer_mat_prop = material_input_data(i+1,:); %retrieve material properties of chosen material index
        end
    end
    ...
    t_array{k} = cell2mat(layer_mat_prop(7));
    ...
    Q_bar_array{k} = Q_bar; %Q_bar_array arranged from 1st play as 1 and nth ply as nth
end

%Q_bar_array_Nto1 = fliplr(Q_bar_array_1toN);
z = ply_edges_nonuniform(t_array); % Generate z stack
